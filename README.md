# Offer之路

一个简洁的求职投递记录管理工具，帮助你高效追踪求职进度。
## 页面
<img width="2549" height="1242" alt="image" src="https://github.com/user-attachments/assets/790625b1-1e1b-42a3-b998-93d0407c6ba7" />
<img width="2549" height="1242" alt="image" src="https://github.com/user-attachments/assets/f67c2fac-3297-4594-ae37-f5c63d62d178" />
<img width="2549" height="1242" alt="image" src="https://github.com/user-attachments/assets/516d35f9-3149-49c8-b930-814d5d6976e3" />
<img width="2549" height="1242" alt="image" src="https://github.com/user-attachments/assets/0cdf6061-4950-4051-badc-1056d8fcb3f6" />
<img width="2560" height="1304" alt="image" src="https://github.com/user-attachments/assets/146d91bd-4fbd-4a28-9ffe-30c4079836f1" />


## 功能特性

### 投递记录管理
- **新增记录**：填写公司名、岗位、城市、投递链接、状态和备注
- **编辑记录**：随时修改已有的投递信息
- **删除记录**：移除不需要的记录（带确认提示）
- **网格展示**：卡片式网格布局，桌面端一行三列，移动端一行两列
- **状态筛选**：按状态快速筛选查看投递记录
- **搜索**：按公司名模糊匹配搜索

### 状态管理
支持7种投递状态，不同状态显示不同背景色区分：
- 🔘 未投递 — 灰色
- 🔵 已投递 — 蓝色
- 🟡 笔试中 — 黄色
- 🟣 面试中 — 粉紫色
- 🔴 挂了 — 红色
- 🔵 等offer — 天蓝色
- 🟢 收到offer — 绿色

### 设置功能
- 设置默认岗位名称，新增记录时自动填充

### 统计面板
实时显示投递数据概览：
- 总投递数
- 笔试中
- 面试中
- 挂了
- 等offer
- Offer

### 导出报告
支持导出为 TXT 或 Markdown 格式的投递报告：
- 包含统计概览
- 按状态分类的详细记录
- 自动下载到浏览器

## 快速开始

### 方式：手动打开
在浏览器中直接打开 `index.html` 文件即可使用。

## 文件结构

```
OFFER之路/
├── index.html          # 主页面
├── style.css           # 样式文件
├── app.js              # 核心逻辑
├── start.bat           # 快速启动脚本
└── README.md           # 项目说明
```

## 数据存储

- 数据保存在浏览器本地存储（localStorage）中
- 数据自动保存，无需手动操作
- 清除浏览器缓存会导致数据丢失，建议定期导出报告备份

## 技术栈

- **前端**：HTML5 + CSS3 + JavaScript ES6+
- **样式**：明亮轻快主题
- **图标**：Lucide Icons（SVG）
- **字体**：Inter（Google Fonts）
- **数据**：浏览器 localStorage

## 使用说明

1. 点击「新增投递」按钮添加记录
2. 填写公司名、岗位、城市等信息
3. 选择当前投递状态
4. 备注会自动填充日期格式（如：2026-8-27投递）
5. 点击「编辑」修改记录，点击「删除」移除记录
6. 使用状态筛选或搜索框快速定位特定记录
7. 点击「导出报告」生成投递总结

## 响应式设计

支持桌面端和移动端访问，布局自动适配屏幕尺寸。
